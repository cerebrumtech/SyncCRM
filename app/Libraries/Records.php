...
class Records
{
    ...
    /**
     * Types a browser may render in place without being able to run script.
     * SVG is deliberately absent: it is an XML document that can carry <script>,
     * so shown inline it executes on this origin with the viewer's session.
     */
    ...
    /**
     * The real type of a file on disk, never the one the uploader claimed.
     * A browser sends Content-Type in the upload, so trusting it lets anyone
     * choose how their file is served back.
     */
    public static function detectMime(string $absPath, string $fallback = 'application/octet-stream'): string
    {
        if (function_exists('finfo_open')) {
            $fi = finfo_open(FILEINFO_MIME_TYPE);
            if ($fi !== false) {
                $found = finfo_file($fi, $absPath);
                finfo_close($fi);
    ...
    /** True when this type is safe to show in the page rather than download. */
    public static function isInlineSafe(string $mime): bool
    {
        return in_array(strtolower($mime), self::INLINE_SAFE, true);
    }
