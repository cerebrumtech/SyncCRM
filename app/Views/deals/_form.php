...
<div class="grid gap-3 sm:grid-cols-2">
  ...
  <div class="field"><label class="label">Contact</label><div data-picker="/api/search/contacts" data-create="/api/quick-create/contacts" data-create-label="Add contact" data-name="contact_id" data-value="<?= esc(old('contact_id') ?? $d['contact_id'] ?? $prefill['contact_id'] ?? '', 'attr') ?>" data-label="<?= esc(old('contact_id') ? '' : (isset($contact) && $contact ? full_name($contact) : ($prefill['contact_label'] ?? '')), 'attr') ?>" data-placeholder="Search contacts…"></div></div>
  <div class="field"><label class="label">Company</label><div data-picker="/api/search/companies" data-create="/api/quick-create/companies" data-create-label="Add company" data-name="company_id" data-value="<?= esc(old('company_id') ?? $d['company_id'] ?? $prefill['company_id'] ?? '', 'attr') ?>" data-label="<?= esc(old('company_id') ? '' : ($company['name'] ?? $prefill['company_label'] ?? ''), 'attr') ?>" data-placeholder="Search companies…"></div></div>
