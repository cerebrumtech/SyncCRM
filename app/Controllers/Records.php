...
class Records extends BaseController
{
    ...
    public function file(int $attId)
    {
        ...
        $inline = str_starts_with($att['mime_type'], 'image/') || $att['mime_type'] === 'application/pdf' || str_starts_with($att['mime_type'], 'text/');
        return $this->response->setHeader('Content-Type', $att['mime_type'])
            ->setHeader('Content-Disposition', ($inline ? 'inline' : 'attachment') . '; filename="' . str_replace('"', '', $att['filename']) . '"')
