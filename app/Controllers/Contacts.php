...
class Contacts extends BaseController
{
    ...
    private function readForm(): array
    {
        ...
        // A number that cannot be normalised gets no duplicate-detection key, so say so
        // at the point of entry rather than saving a contact that silently skips dedupe.
        foreach (['phone' => $phone, 'whatsapp_number' => $whatsapp] as $label => $value) {
            if ($value !== null && $value !== '' && normalize_phone($value) === null) {
                $this->fail(($label === 'phone' ? 'Phone' : 'WhatsApp') . ' number looks wrong. Use a 10-digit Indian mobile, or include the country code.');
            }
        }
        return [
